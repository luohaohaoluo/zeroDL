...
def bbox_iou(bbox_a, bbox_b):
    # ---------------------------------------------------------------------------
    # bbox_a传入的是单个样本的所有先验框，bbox_b传入单个样本的真实框！！！！
    # 假设：bbox_a和bbox_b的shape分别为(N,4), (M,4)
    # 这里的真实框不要想成是一个框，因为一张图片可能存在多个物体，每一个先验框都需要计算一遍
    # ---------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------
    # 计算交集面积
    # (tl < br).all(axis=2): 筛选满足(tl < br)要求的数值
    # -------------------------------------------------------------
...
"""
将一个边界框（src_bbox）转换为目标边界框（dst_bbox）的偏移量
"""
...
class AnchorTargetCreator(object):
    ...
    def __call__(self, bbox, anchor):
        # 为每个先验框找到对应的最大的真实框的序号，以及每个先验框的标签
    ...
    def _calc_ious(self, anchor, bbox):
        ...
        # ---------------------------------------------------------------------#
        #   保证每一个真实框都存在对应的先验框
        #   可能存在某些真实框没有被分配到任何先验框（因为先验框可能已经分配给了其他真实框）
        # ---------------------------------------------------------------------#
    ...
    def _create_label(self, anchor, bbox):
        ...
        #   1是正样本，0是负样本，-1忽略
        #   初始化的时候全部设置为-1
        #   anchor.shape ---> (1764, 4), len(anchor) --->1764
        # ------------------------------------------------------- #
        ...
        #   如果小于门限值则设置为负样本
        #   如果大于门限值则设置为正样本
        #   每个真实框至少对应一个先验框, 怕出现max_ious小于阈值的情况,导致某一个真实框没有对应的先验框
        # ------------------------------------------------------------------------------------ #
...
class ProposalTargetCreator(object):
    ...
    def __call__(self, roi, bbox, label, loc_normalize_std=(0.1, 0.1, 0.2, 0.2)):
        ...
        '''
        # roi ---> (600, 4), bbox --> (2, 4)
        # print(roi.shape,  bbox.shape)
        #  为了在后续计算中统一处理提议框和真实框的 IoU（交并比）,将roi与真实框合并
        ...
        # ---------------------------------------------------------------------- #
        #   计算建议框和真实框的重合程度
        #   iou  --> (num_roi+num_bbox, num_bbox) 表示每个建议框与每个真实框的IoU
        # ---------------------------------------------------------------------- #
        ...
        # ----------------------------------------------------------------#
        #   满足建议框和真实框重合程度大于等pos_iou_thresh的作为正样本
...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale):
        # ---------------------------------------------------------- #
        #   imgs.shape    ---> (batch, channel, height, width)
        #   bboxes.shape  ---> (batch, 4)
        #   labels.shape  ---> (batch, )
        # ---------------------------------------------------------- #
        ...
        for i in range(n):
            ...
            # 每个建议框其实对应了一个标签，此时只需要将每个建议框预测的对应那个标签结果的偏移量取出来就行了
        ...
        # 最后返回一个包含五个元素的列表, (RPN定位损失, RPN分类损失, ROI定位损失, ROI分类损失, 所有损失项的和)
...
def get_lr_scheduler(lr_decay_type, lr, min_lr, total_iters, warmup_iters_ratio=0.05, warmup_lr_ratio=0.1,
    ...
    def yolox_warm_cos_lr(lr, min_lr, total_iters, warmup_total_iters, warmup_lr_start, no_aug_iter, iters):
        # -----------------------------------
        #  Warm-up 和余弦退火策略
        #  当epoch小于等于warmup_total_iters，采取 Warm-up
        #  当epoch大于等于total_iters - no_aug_iter，就用设定好的最小学习率
        #  其余epoch用余弦退火方法
        # -----------------------------------
