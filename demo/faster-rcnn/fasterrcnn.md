...
- **2**.从forwad()开始看，首先是根据backbone的feature map，经过简单的卷积操作，得到两条分支，分别是**rpn_locs**和**rpn_scores**，用于记录**预测框的位置**和**对应的得分**。与此同时，根据得分，筛选出有物体的预测框出来做准备。值得提及的是根据后来的代码查阅，发现，其实**rpn_locs**的最后一维4个参数分别代表(**左上x坐标，左上y坐标，以左上x为基准的W，以左上y为基准的H**)，所谓的预测框，其实更像是在先验框的基础上，通过参数化的微调从而实现的。**这里可以看出双端目标检测网络，其实是需要先验框的配合，结合预测数值搭配完成预测。**
